...
_DEFAULT_MONITOR = "val_loss"
_DEFAULT_MODE = "min"
_DEFAULT_SAVE_TOP_K = 3
_DEFAULT_FILENAME = "epoch={epoch}-valloss={val_loss:.4f}"
...
def build_callbacks(cfg_like: Any, *, checkpoint_dir: Optional[str] = None) -> List[Callback]:
    ...
    if cfg.visualization_enabled:
        from .visualization import SequenceHeatmapVisualizationCallback

        callbacks.append(
            SequenceHeatmapVisualizationCallback(
                max_sequences=cfg.visualization_max_images,  # Re-interpreting max_images as max_sequences
