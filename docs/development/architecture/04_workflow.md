...
4.  **モデルと学習ロジックの実装**
    ...
    - `model/`: `nn.Module`として、ごく単純なモデルアーキテクチャを定義します。
      - **推奨**: `if __name__ == '__main__':` ブロックを追加し、モデル単体でダミーデータを入力してフォワードパスが通るかを確認するコードを記述します。これにより、後のデバッグが容易になります。
...
1.  **機能の拡張**
    ...
    - **カスタム機能の実装**: 必要に応じて、以下のディレクトリを作成し、機能を分離・実装します。
      ...
      - `callbacks/`: 学習の途中経過の可視化や、特定の条件でのカスタムロギングなど、プロジェクト固有のコールバック。
